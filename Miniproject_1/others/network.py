...
class UNetSmall32(nn.Module):
    def __init__(self, in_channels, out_channels, cut_last_convblock):
        super(UNetSmall32, self).__init__()
        self.out_channels = out_channels
        self.in_channels = in_channels
        self.in_channelsx2 = in_channels * 2
        self.cut_last_convblock = cut_last_convblock
        ...
        self._block6 = nn.Sequential(
            nn.Conv2d(self.in_channelsx2 + out_channels, 32, (3, 3), padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 16, (3, 3), padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, out_channels, (3, 3), padding=1),
        )
...
class UNetSmallNoSkip(nn.Module):
    def __init__(self, in_channels, out_channels, cut_last_convblock):
        ...
        self._block6 = nn.Sequential(
            nn.Conv2d(self.in_channelsx2, 64, (3, 3), padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 32, (3, 3), padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, out_channels, (3, 3), padding=1),
...
def build_network(config):
    ...
    elif config["model"] == 'unet_small_32':
        net = UNetSmall32(in_channels=32, out_channels=3, cut_last_convblock=False)
    else:
