...
def __create_transformer(self, mean, std):
    ...
    if "translate" in self.augmentations:
        transformers.append(T.RandomAffine(degrees=0, translate=tuple(*self.augmentations["translate"])))
    if "scale" in self.augmentations:
        transformers.append(T.RandomAffine(degrees=0, scale=tuple(*self.augmentations["scale"])))
...
def augment_data(self, images, targets):
    ...
    return transformer(images), transformer(targets)
